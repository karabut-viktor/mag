...
import ee.ut.solmir.mag.act.factory.ACTFactory;
...
public class App {
  ...
  private static class SimHolder {
  ...
  private static void mostSimilarMethdos(String fileName) {
    ...
    List<SimHolder> holders = new ArrayList<App.SimHolder>();
    for (int i = 0; i < methods.size() - 1; i++) {
      for (int j = i + 1; j < methods.size(); j++) {
        ...
        if (sim != 1.0) {
          holders.add(new SimHolder(m1, m2, m1.similarity(m2)));
        }
      }
    }
